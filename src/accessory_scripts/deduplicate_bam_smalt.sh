...
# Example: sh deduplicate_bam_smalt.sh
# ../../../tn-seq_data/Quintara_NGS_Raw/2021-11-15_tn107-109_p146_arbutin_MiSeq/Fastq/mae-001_metadata.txt
# ../../../tn-seq_data/tn-seq_outputs/mae-001/smalt_output/*_L001.bam


### OUTPUTS
## SMALT + SAMTOOLS
## ${smalt_out_path}.deduplicated.bam: bam file deduplicated by UMI and alignment
...
smalt_out_path=$dname/$fname
  ...
  smalt_out_path=../../../tn-seq_data/tn-seq_outputs/$experiment_name/smalt_output_dedup/${fname}
  ...
  if [[ "$bam" == "$1" ]]
    then
      if [ ! -d "../../../tn-seq_data/tn-seq_outputs/$experiment_name/smalt_output_dedup/" ]
      then
        mkdir ../../../tn-seq_data/tn-seq_outputs/$experiment_name/smalt_output_dedup/
      fi
...
#
  ...
  samtools sort $bam -@ $nproc -o ${smalt_out_path}.sorted.bam
  echo "Indexing bam file"
  samtools index ${smalt_out_path}.sorted.bam

  echo "Deduplicating bam file"
  umi_tools dedup  --umi-separator=: --paired --log=${smalt_out_path}_dedup_log \
  -I ${smalt_out_path}.sorted.bam --output-stats=${smalt_out_path}_deduplicated -S \
  ${smalt_out_path}.deduplicated.bam

  echo "Cleaning up "
  rm ${smalt_out_path}.sorted.bam
  rm ${smalt_out_path}.sorted.bam.bai
...
python deduplication_stats.py -i ../../../tn-seq_data/tn-seq_outputs/$experiment_name/smalt_output_dedup/
